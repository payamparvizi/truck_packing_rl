...
from packages.tianshou.data import (
    Batch,
    CachedReplayBuffer,
...
from packages.tianshou.data.batch import _alloc_by_keys_diff
from packages.tianshou.env import BaseVectorEnv, DummyVectorEnv
from packages.tianshou.policy import BasePolicy
...
class Collector(object):
    ...
    def collect(
        ...
        while True:
            ...
            ptr, ep_rew, ep_len, ep_idx, ep_dens, ep_fdens = self.buffer.add(
            ...
            if np.any(done):
                ...
                episode_density.append(ep_dens[env_ind_local])
                episode_final_density.append(ep_fdens[env_ind_local])
        ...
        if episode_count > 0:
            rews, lens, idxs, density, final_density = list(
                map(
                    np.concatenate,
                    [episode_rews, episode_lens, episode_start_indices, episode_density, episode_final_density]
                )
            )
