...
from packages.tianshou.data import Batch
from packages.tianshou.data.batch import _alloc_by_keys_diff, _create_value
from packages.tianshou.data.utils.converter import from_hdf5, to_hdf5
...
class ReplayBuffer:
    ...
    def __init__(
        ...
        **kwargs: Any,  # otherwise PrioritizedVectorReplayBuffer will cause TypeError
        ...
        self._ep_dens: Union[float, np.ndarray]
        self._ep_fdens: Union[float, np.ndarray]
        self.reset()
    ...
    def reset(self, keep_statistics: bool = False) -> None:
        ...
        if not keep_statistics:
            self._ep_rew, self._ep_len, self._ep_idx, self._ep_dens, self._ep_fdens = 0.0, 0, 0, 0.0, 0.0
    ...
    def _add_index(self, rew: Union[float, np.ndarray],
                   ...
                   dens: Union[float, np.ndarray],
                   fdens: Union[float, np.ndarray]) -> Tuple[
                    int,
        ...
        self._ep_dens += dens
        self._ep_fdens += fdens

        if done:
            result = ptr, self._ep_rew, self._ep_len, self._ep_idx, self._ep_dens, self._ep_fdens
            self._ep_rew, self._ep_len, self._ep_idx = 0.0, 0, self._index
            self._ep_dens = 0.0
            self._ep_fdens = 0.0
            return result
        else:
            return ptr, self._ep_rew * 0.0, 0, self._ep_idx, self._ep_dens * 0.0, self._ep_fdens * 0.0
    ...
    def add(
        ...
        if stacked_batch:
            ...
            dens = batch.info["denisty"][0]
            fdens = batch.info["final_denisty"][0]
        else:
            rew, done = batch.rew, batch.done
            dens = batch.info["denisty"]
            fdens = batch.info["final_denisty"]
        ptr, ep_rew, ep_len, ep_idx, ep_dens, ep_fdens = list(
            map(lambda x: np.array([x]), self._add_index(rew, done, dens, fdens))
        )
        try:
            ...
            self._meta[ptr] = batch
        return ptr, ep_rew, ep_len, ep_idx, ep_dens, ep_fdens 
