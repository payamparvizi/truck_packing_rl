...
from packages.tianshou.data import Batch, HERReplayBuffer, PrioritizedReplayBuffer, ReplayBuffer
...
class ReplayBufferManager(ReplayBuffer):
    ...
    def add(
        ...
        for batch_idx, buffer_id in enumerate(buffer_ids):
            ptr, ep_rew, ep_len, ep_idx, ep_dens, ep_fdens = self.buffers[buffer_id]._add_index(
                batch.rew[batch_idx], batch.done[batch_idx], batch.info.current_density[batch_idx], 
                batch.info.final_density[batch_idx]
            )
            ptrs.append(ptr + self._offset[buffer_id])
            ep_lens.append(ep_len)
            ep_rews.append(ep_rew)
            ep_denss.append(ep_dens)
            ep_fdenss.append(ep_fdens)
        ...
        try:
            ...
            self._set_batch_for_children()
            self._meta[ptrs] = batch
        return ptrs, np.array(ep_rews), np.array(ep_lens), np.array(ep_idxs), np.array(ep_denss), np.array(ep_fdenss)
